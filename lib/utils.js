...
function getItem(item){
	...
	return fetch(`http://34.73.193.139:8080/api?q=${item}&zip=${zip}`).then(function(resp){
		return resp.json();
	})
}
...
async function getUser(user){
	...
	return await fetch("http://34.73.193.139:8080/api/login",
		{method: "POST",
		cache: 'no-cache',
		headers: {
...
async function handleCreate(user){
	const params = new URLSearchParams(user);
	return await fetch("http://34.73.193.139:8080/api/create-user",
		{method: "POST",
		cache: 'no-cache',
		headers: {
			'Accept': 'application/json',
